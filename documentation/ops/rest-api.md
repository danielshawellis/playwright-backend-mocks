...
HAR is available only when a response body was recorded:

- **`fulfill`** (`outcome.kind === "mocked"`) — always
- **`continue`** — only if an upstream response was attached to the history entry (v1 usually records the continue decision + overrides without the upstream body)
