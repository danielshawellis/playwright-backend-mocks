// TODO(Step 2): target API is routeFromHAR (Playwright oracle). This JSON
// cassette helper is retained so existing callers keep building; prefer HAR.
...
function createReplayHandler(
  ...
  return async (route, request) => {
    ...
    if (entry === undefined) {
      ...
      // Mirror Playwright HarRouter notFound: 'fallback'.
