...
| `continue` (+ url/method/headers/postData, JSON/binary/empty/long/utf8, forbidden headers)          | `continue.spec.ts`                                      | covered |
...
| `route.fetch` (+ timeout/0, maxRedirects, maxRetries, signal, overrides, gzip, CT defaults)         | `fetch.spec.ts`                                         | covered |
| `APIResponse` ok / json throw / dispose / statusText / headersArray / body                          | `fetch.spec.ts`                                         | covered |
| Matchers: glob / RegExp / predicate / URLPattern                                                    | `matchers.spec.ts`                                      | covered |
| Glob edges (`?`, `*` vs `**`, `{a,b}`, entire-URL, backslash escape, encoding, baseURL, Disposable) | `matchers.spec.ts`                                      | covered |
| `times` (+ exhaust, LIFO stacking, fallback consumes a use)                                         | `times.spec.ts`                                         | covered |
...
| `waitForResponse` (string/RegExp/predicate/timeout/0/continue)                                      | `wait-for-response.spec.ts`                             | covered |
| Request inspection (postData/JSON/form/headers/response/failure/inspect-then-continue)              | `inspection.spec.ts`                                    | covered |
...
| Aborted update entries are failure-marked / not fulfillable | covered |
